...
namespace kv
{
    ...
    // This is what happened when a new node just starts from scratch
    RawNode::RawNode(const Config &conf, const std::vector<PeerContext> &peers)
    {
        ...
        if (last_index == 0)
        {
            ...
            // Why is the node assuming that these entries are stable in storage memory?
            // Because this info is init through goreman and all other nodes know the peer info
    ...
    bool RawNode::has_ready()
    {
        ...
        if (!raft_->msgs_.empty() || !raft_->raft_log_->unstable_entries().empty() || raft_->raft_log_->has_next_entries())
        {
            return true;
        }
