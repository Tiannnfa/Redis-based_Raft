...
namespace kv
{
    ...
    // This creates an instance of RedisStore with RaftNode::io_service_, port_ and its snap_data_
    // The RedisStore instance will have snapdata in its RedisStore::key_values
    // The acceptor_ will listen to any available IP on the given port
    ...
    // This function starts the RedisStore server through another thread
    ...
    // This will create a RedisSession that uses RaftNode::io_service_ to accept requests
    // The RedisSession has a class member redisReader* from HiRedis library working as a parser
    // Continually start_accpet() once it establishes a connection
