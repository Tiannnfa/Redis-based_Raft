...
namespace kv
{
    ...
    public:
        ...
        // According the the WAL_type and data provided, first generate a WAL_Record
        // then append both the WAL_record and the data into the WAL_File::data_buffer
        ...
        // Write the content in the WAL_File::data_buffer(which is in the memory) into the actual file on disk
        // This is the consistency step.
        void sync()
        {
    ...
    // Create the WAL_File instance with the directory such that we can open WAL instance 
    // The WAL_File has a format "sequence-index" with initial sq-idx being 0-0
    // When creating, the snapshot term and index, which is both 0, is appended to the WAL_File and synchronized
    ...
    // Find the WAL_File(s) which contains the centent after last snapshot
    // Initialize WAL instance with these WAL_Files
    // Initizlize WAL::start_ with given WAL_Snapshot, representing the WAL effectively start from given index and term
    ...
    // Read all the file content within the WAL directory according to WAL_Record type
    // If the content is not equal to the length written in WAL_Record we have to truncate the file
    // matchsnap is set if any file in the directory matches the snapshot index and term(which is WAL_Snapshot)
    // hardstate and ents are also being read out of the file
    ...
    // Get the sequence number and index of a .wal file
    ...
    // Return true if the given files are sequentially-numbered(e.g.No missing sequence)
