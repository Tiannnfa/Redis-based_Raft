...
namespace kv
{
    namespace proto
    {
        ...
        {
            return type == MsgHup || type == MsgBeat || type == MsgUnreachable || type == MsgSnapStatus || type == MsgCheckQuorum;
        }

        bool Message::is_response_msg() const
        {
            return type == MsgAppResp || type == MsgVoteResp || type == MsgHeartbeatResp || type == MsgUnreachable || type == MsgPreVoteResp;
        }
        ...
        {
            return 1 + u8_serialize_size(type) + u64_serialize_size(term) + u64_serialize_size(index) + data_serialize_size(static_cast<uint32_t>(data.size()));
        }
