...
#define RECEIVE_BUFFER_SIZE (1024 * 512)
    ...
    // Start to read the content from given socket
    // Once data from Redis clients has been read into the RedisSession::read_buffer_ from the socket
    // It starts to handle the data by invoking RedisSession.handle_read()
    // RedisSession.handle_read() will use the RedisReader parser to parse the syntax from the Redis clients
    // According to the parsed request, it invokes corresponding function through a functor
