...
namespace kv
{
          ...
          max_next_ents_size_(max_next_ents_size)
    {
        ...
        // Compaction is where last snapshot happens
    ...
    // Check the entries from messages are compatible with the entries in Unstable entries or not
    ...
    // Set the applied_ fielf of RaftLog in Raft class when restarting a Node(RawNode)
    ...
    // According to the content in Unstable or MemoryStorage try to get the term
    // Note that Unstable has all the entries that are not yet written to storage
    Status RaftLog::term(uint64_t index, uint64_t &t) const
    {
        uint64_t dummy_index = first_index() - 1;
        // This is the case in which index is out of boundary since last snapshot
    ...
    // Firstly try to get the index through RaftLog::unstable_, if Ok, returns
    // Then try to get the index through memorystorage_ if snapshot_ is nullptr
    ...
    // Logic same as RaftLog::first_index()
    // Firstly try to get the index through RaftLog::unstable_, if Ok, returns
    // Then try to get the index through memorystorage_ if snapshot_ is nullptr
