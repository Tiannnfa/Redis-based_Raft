...
namespace kv
{
    ...
    // Bind the step_follower() function to function pointer step_
    // Bind tick_election() to function pointer tick_
    // Also initialize the lead_ filed of Raft
        ...
        case proto::MsgReadIndex:
        {
            if (quorum() > 1)
            {
                ...
                case ReadOnlyLeaseBased:
                    ...
                    { // from local member
                        read_states_
                            .push_back(ReadState{.index = raft_log_->committed_, .request_ctx = msg->entries[0].data});
                    }
                    else
                    {
                        proto::MessagePtr m(new proto::Message());
    ...
    {
        msg->from = id_;
        if (msg->type == proto::MsgVote || msg->type == proto::MsgVoteResp || msg->type == proto::MsgPreVote || msg->type == proto::MsgPreVoteResp)
        {
            if (msg->term == 0)
            {
    ...
    // This function returns true if itself is in prs_
    ...
    // This is to add a node into the cluster
    ...
    // This function processes message related to Raft mechanism
    Status Raft::step_follower(proto::MessagePtr msg)
    {
        switch (msg->type)
        {
        // Propagate the incoming message to the leader
        case proto::MsgProp:
            if (lead_ == 0)
            {
                LOG_INFO("%lu no leader at term %lu; dropping proposal", id_, term_);
            ...
            send(msg);
            break;
        // Append entries to RaftLog::Unstable
        case proto::MsgApp:
        {
            election_elapsed_ = 0;
            lead_ = msg->from;
    ...
    // As per the request of the leader(might-be outdated leader, previous leader), append entries to raft_log_::
    // First check the index within the message, if not up-to-date, send back a message containing current max index and term
    // Then read the entries from the message and try to append to the RaftLog::Unstable
    void Raft::handle_append_entries(proto::MessagePtr msg)
    {
        ...
        if (ok)
        {
            ...
            m->to = msg->from;
            m->type = proto::MsgAppResp;
            // last_index is the last index in Unstable
            m->index = last_index;
            send(std::move(m));
        }
        else
    ...
    // Load the hardstate of Raft including committed_, term_, vote_
    ...
    // Set the progress state to be a learner or a voter
    // Other fields like match, next could be set here
    ...
    // Simple wrap of finding quorum index and try to commit through raft_log_
    // Here it seems that by "commit" we simply set committed_ field
    ...
    // Reset term if non-initialization
    // When initializing, set all related fields to be 0, most importantly set random randomlized_election_timeout,
    // abort all leader transfer, create Progress instance hereby by setting up Progress::next to be last index of raft_log_ + 1,
    // Progress::match to be last index of raft_log_
    ...
    // Encapsulation for adding a voter node or a learner node
    ...
    // This function increases election_elapsed_
    ...
    // This function returns true if election timeout
