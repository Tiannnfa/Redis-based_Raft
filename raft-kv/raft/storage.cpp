...
namespace kv
{
    ...
    // The hard_state_ is initially from WAL file and conf_state from Snap file
    ...
    // This the index of the last actual entry content,
    // the first entry has no data but just metadata like term and index
    // Note these entries are stable because they have been written to storage_
    ...
    // This is the index of the first actual entry content since last snapshot happens,
    // the first entry has no data but just metadata like term and index
    // Note these entries are stable because they have been written to storage_
    ...
    // This function appends the storage entries, which is stable
    Status MemoryStorage::append(std::vector<proto::EntryPtr> entries)
    {
        ...
        // This is to calculate the might-be last actual entry index
    ...
    // Apply snapshot through MemoryStorage
    // Initialize the MemoryStorage::snapshot_ with the argument snapshot
    // The MemoryStorage::entries_ is the entries since last snapshot with first entry being the term and index the snapshot was taken
    ...
    // This is the index of the first actual entry content, 
    // the first entry has no data but just metadata like term and index
