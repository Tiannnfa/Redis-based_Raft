...
namespace kv
{
    ...
    // RaftNode is inherited from RaftServer, it has its own io_service_
    // When initialize it initializes port, timer_, id_, conf_state_(which contains nodes and learners id),
    // storage_(which has access to hard_state_ , snapshot_ and entries), last_index_, snapshot_index_,
    // applied_index_.
          ...
          snap_count_(defaultSnapCount)
    {
        ...
        // This is used to denote the initial existence of wal_dir_ 
        // since later on we will create the wal_dir_ anyway
    ...
    // According to the snapshot provided, create one WAL_File instance if not existed
    // Then with the newly created WAL_File(Meaning non-empty snapshot directory) we can create and open a WAL instance
    ...
    // First load the most recent snapshot file accordingly(including snapshotdata and snapshot metadata) through snapshotter_
    // Then apply the snapshot content through storage_
    void RaftNode::replay_WAL()
    {
        LOG_DEBUG("replaying WAL of member %lu", id_);

        proto::Snapshot snapshot;
        // Load snapshot content from snapshotter directory
        ...
        {
            // Apply the snapshot to memory storage
        ...
        proto::HardState hs;
        std::vector<proto::EntryPtr> ents;
        // Read all the entries from the WAL dating back to last snapshot was taken
    ...
    void RaftNode::main(uint64_t id, const std::string &cluster, uint16_t port)
    {
        ...
        std::string &host = g_node->peers_[id - 1]; // host is a string of IP address and port, representing the node itself
